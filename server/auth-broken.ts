...
export async function getUserPermissions(userEmail: string, accessToken: string): Promise<UserPermissions> {
  ...
  // If user is not in any access list, deny access by returning empty permissions with restrictive flag
  ...
  if (!hasAnyAccess) {
    ...
    // Return permissions that will result in no data access
    return {
      hasFullAccess: false,
      allowedDepartments: [],
      allowedClients: ['NO_ACCESS_GRANTED'], // Special flag to deny all access
      userEmail: normalizedEmail
    };
  }

  // Legacy SharePoint integration - only for development fallback
