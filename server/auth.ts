...
export async function getUserPermissions(userEmail: string, accessToken: string): Promise<UserPermissions> {
  ...
  if (CLIENT_BASED_USERS.includes(normalizedEmail)) {
    ...
    // For timesheet.admin, grant full access to all clients temporarily while fixing SharePoint integration
    if (normalizedEmail === 'timesheet.admin@royalcyber.com') {
      console.log(`🎯 Granting full access to timesheet.admin while SharePoint integration is resolved`);
      permissions.hasFullAccess = true;
      permissions.allowedClients = [];
      permissions.allowedDepartments = [];
      console.log(`✅ Full access granted to timesheet.admin`);
    } else {
