...
export class AzureSqlStorage implements IStorage {
  ...
  async getEmployees(filter?: EmployeeFilter): Promise<{
    ...
    try {
      ...
      // Simplified working query with proper business logic (targeting 251 employees)
      const employeeQuery = `
        WITH EmployeeData AS (
          ...
          LEFT JOIN (
            SELECT ztl.UserName, ztl.Date, ztl.BillableStatus, ztl.Project,
        ...
        SELECT 
          ...
          -- Client name deduplication: Remove duplicate patterns like "| |"
          CASE 
            WHEN ClientName IS NULL OR LTRIM(RTRIM(ClientName)) = '' THEN 'No Client'
            ELSE REPLACE(REPLACE(LTRIM(RTRIM(ClientName)), ' |  | ', ' | '), '| |', ' | ')
          END as client,
          COALESCE(ClientName, 'No Client') as clientSecurity,
          COALESCE(ProjectName, 'No Project') as project,
          ...
          CASE 
            ...
            WHEN BillableStatus = 'Non-Billable' THEN 'Non-Billable ≤10 days'
            ELSE 'No timesheet filled ≤10 days'
          END as nonBillableAging,
          -- Standard Timesheet Aging
          CASE 
            WHEN LastTimesheetDate IS NULL THEN '90+'
            WHEN DATEDIFF(DAY, LastTimesheetDate, GETDATE()) >= 91 THEN '90+'
            WHEN DATEDIFF(DAY, LastTimesheetDate, GETDATE()) >= 61 THEN '61-90'
            WHEN DATEDIFF(DAY, LastTimesheetDate, GETDATE()) >= 31 THEN '31-60'
            ELSE '0-30'
