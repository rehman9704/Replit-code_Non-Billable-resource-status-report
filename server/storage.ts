      ...
      const query = `
        ...
        MergedData AS (
          ...
          LEFT JOIN (
              ...
              FROM RC_BI_Database.dbo.zoho_TimeLogs WITH (NOLOCK)
              WHERE Date >= DATEADD(MONTH, -3, GETDATE())  -- OPTIMIZED: Recent status for performance
              GROUP BY UserName
          ...
          WHERE 
              ...
              AND a.[CostPerMonth(USD)] IS NOT NULL  -- CRITICAL: Only include employees with cost data to get exactly 245
